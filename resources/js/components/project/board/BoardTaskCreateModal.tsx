...
import { Project, User, Label as LabelType, Tag } from '@/types/project-manager';
...
interface BoardTaskCreateModalProps {
    open: boolean;
...
export default function BoardTaskCreateModal({
    open,
    onOpenChange,
    project,
    list,
    ...
    const handleCreateTask = (e: React.FormEvent) => {
        ...
        const taskData = {
            ...createTaskData,
            status: 'to_do', // Default status for board tasks
        };

        console.log('Creating task with data:', taskData);
        console.log('Using route:', route('tasks.store', {
            project: project.id,
            board: list.board_id,
            list: list.id
        }));
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[500px]">
                <DialogHeader>
                    <DialogTitle>Create New Task in {list.name}</DialogTitle>
                </DialogHeader>
